...
// ============================================================================
// CDM DE-RISK: verify a REAL proof generated by our cdm-guest (Merkle + COUNT + k-anon)
// Reads proof.txt produced by /work/derisk/cdm-guest/host. Lines: seal / image_id / journal_digest.
// ============================================================================
#[test]
fn test_verify_cdm_query_proof() {
    let (env, client) = setup_test();

    let content = std::fs::read_to_string("/work/derisk/cdm-guest/proof.txt")
        .expect("run the cdm-guest host first to produce proof.txt");
    let mut lines = content.lines();
    let seal_bytes = hex::decode(lines.next().unwrap()).unwrap();
    let image_id_bytes: [u8; 32] = hex::decode(lines.next().unwrap()).unwrap().try_into().unwrap();
    let journal_digest_bytes: [u8; 32] =
        hex::decode(lines.next().unwrap()).unwrap().try_into().unwrap();

    let seal = Bytes::from_slice(&env, &seal_bytes);
    let image_id = BytesN::from_array(&env, &image_id_bytes);
    let journal_digest = BytesN::from_array(&env, &journal_digest_bytes);

    // journal_digest in proof.txt is ALREADY sha256(journal), which is what verify() expects.
    assert_eq!(client.verify(&seal, &image_id, &journal_digest), ());
    std::println!("\n✅ CDM guest proof verified by the Stellar Groth16 verifier contract\n");
}
